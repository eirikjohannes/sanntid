...
func CollectCosts(costReply <-chan def.Message) {
	...
	for {
		...
		case message := <-costReply:
			handleCostReply(requestMap, message, def.OnlineElevators, timeout)
		case <-timeout:
			log.Println(def.ColR, "Not all costs received in time!", def.ColN)
			chooseBestElevator(requestMap, def.OnlineElevators, true)
		}
	}
}
...
func chooseBestElevator(requestMap map[request][]reply, numOnline int, isTimeout bool) {
	...
	for request, replyList := range requestMap {
		if len(replyList) == numOnline || isTimeout {
			...
			queue.AddOrder(request.floor, request.button, bestElevator)
			request.timer.Stop()
