...
func InitUDP(NetworkToQueueOrderChannel chan def.ElevatorOrder, QueueToNetworkOrderChannel chan def.ElevatorOrder, NetworkToQueueAliveChannel chan def.ElevatorAliveMessage, QueueToNetworkAliveChannel chan def.ElevatorAliveMessage) {
	...
	go bcast.Reciever(def.UDPPort, orderRx)
	go bcast.Reciever(def.UDPPort, aliveRx)
	...
	/*for {
	select {
	case p := <-ElevatorPeerUpdateChannel:
	...
	*/
	/*case a := <-orderRx:
		fmt.Printf("Received: \n")
		go addOrderToQueue(orderTx, &a, QueueToNetworkOrderChannel, NetworkToQueueOrderChannel)
	case b := <-QueueToNetworkOrderChannel:
		fmt.Printf("NEW ORDER to distribute:\n")
		go distributeOrderToNetwork(orderTx, &b)
	*/
	//}
	//}
}
	...
	for {
		...
		if !order.Ack {
			...
			for addedOrder.Ack != true { //or orderID order.)
				fmt.Println("awating response from queue")
				addedOrder = <-QueueToNetworkOrderChannel
			...
			fmt.Println("RecievedOrder is acked, it is therefore possibly a reply to our order")
			if order.ElevatorId == LocalElevatorId {
				addAck(&order)
				fmt.Println("\n from addOrderToQueue:\n order.ACk=1, order is originally sent from this computer.\n\n")
